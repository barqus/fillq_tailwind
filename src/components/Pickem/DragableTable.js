...
const DragableTable = ({ participants }) => {
    ...
    useEffect(() => {
        ...
        // https://3.123.229.48:8080/api/v1/pickems/107170813
        const fetchData = async () => {
            const result = await axios(
                'https://3.123.229.48:8080/api/v1/pickems/' + localStorage.getItem('twitchCode'),
            );
    ...
    const savePickEms = async () => {
        ...
        if (userAlreadyPosted) {
            await axios.delete(
                'https://3.123.229.48:8080/api/v1/pickems/' + userID, { withCredentials: true }
            ).catch(() => {
                notifyDeleteError()
                return
            })
        ...
        await axios.post('https://3.123.229.48:8080/api/v1/pickems/' + userID, objectToPost, { withCredentials: true })
        .then((res) => {
            if(res.status < 300) {
                notify()
                setUserAlreadyPosted(true)
