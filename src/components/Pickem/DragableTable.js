...
const DragableTable = ({ participants }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const result = await axios(
                'https://fillq-333518.appspot.com/api/v1/participants/pickems/' + localStorage.getItem('twitchCode'),{headers: {Authorization: `Bearer ${token}`} }
            );
    ...
    const savePickEms = async () => {
        ...
        if (userAlreadyPosted) {
            await axios.delete(
                'https://fillq-333518.appspot.com/api/v1/participants/pickems/' + userID, { withCredentials: true,headers: {Authorization: `Bearer ${token}`}  }
            ).catch(() => {
                notifyDeleteError()
                return
            })
        ...
        await axios.post('https://fillq-333518.appspot.com/api/v1/participants/pickems/' + userID, objectToPost, { withCredentials: true,headers: {Authorization: `Bearer ${token}`}  })
        .then((res) => {
            if(res.status < 300) {
                notify()
                setUserAlreadyPosted(true)
